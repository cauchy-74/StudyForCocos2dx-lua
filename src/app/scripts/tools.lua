...
-- cc.FileUtils:getInstance():addSearchPath("res/") -- MyApp has the line 
local textureAtlas = cc.Director:getInstance():getTextureCache():addImage("image/atlas.png")

wingPath = cc.FileUtils:getInstance():fullPathForFilename("wav/sfx_wing.wav")
hitPath = cc.FileUtils:getInstance():fullPathForFilename("wav/sfx_hit.wav")
scorePath = cc.FileUtils:getInstance():fullPathForFilename("wav/sfx_point.wav")
fallPath = cc.FileUtils:getInstance():fullPathForFilename("wav/sfx_die.wav")
uiPath = cc.FileUtils:getInstance():fullPathForFilename("wav/sfx_swooshing.wav")
...
-- audio.preloadMusic(wingPath)
-- audio.preloadSound(hitPath)
-- audio.preloadSound(scorePath)
-- audio.preloadSound(fallPath)
-- audio.preloadSound(uiPath)
...
local function generateSpriteLuaFile()
    -- body
    local atlasFile = io.open("image/atlas.txt", "r")
...
function createCommonBackLayer()
    local layerBg = display.newLayer()
    ...
    if clickedButton ~= nil then
        ...
        elseif clickedButton == g_playButton then
            ...
            if g_initFlag == nil then
                gameScene = require("src.app.scenes.GameScene")
            else
